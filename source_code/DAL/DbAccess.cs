...
namespace DAL
{
    ...
    public class DbAccess
    {
        ...
        public static string ThemBL(BienLai bl)
        {
            ...
            // Kiểm tra xem giá trị của trường MaHocVien đã tồn tại trong bảng GhiDanh hay chưa
            string ghiDanhQuery = "SELECT COUNT(*) FROM ghidanh WHERE MaHocVien = @maHocVien";
            SqlCommand ghiDanhCommand = new SqlCommand(ghiDanhQuery, conn);
            ghiDanhCommand.Parameters.AddWithValue("@maHocVien", bl.MaHocVien);
            int ghiDanhResult = (int)ghiDanhCommand.ExecuteScalar();
            if (ghiDanhResult == 0)
            {
                conn.Close();
                return "Học Viên chưa đăng kí học môn nào";
            }
